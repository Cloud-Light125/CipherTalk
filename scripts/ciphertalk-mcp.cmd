...
set "APP_DIR=%~dp0"
set "EXE_PATH=%APP_DIR%CloudLight WeChat.exe"
...
if not exist "%EXE_PATH%" (
  >&2 echo [CloudLight WeChat MCP Launcher] CloudLight WeChat.exe not found at "%EXE_PATH%"
  exit /b 1
)

if not exist "%MCP_BOOTSTRAP%" (
  >&2 echo [CloudLight WeChat MCP Launcher] MCP bootstrap not found at "%MCP_BOOTSTRAP%"
  exit /b 1
)
...
if exist "%MCP_ENTRY_UNPACKED%" (
  ...
  >&2 echo [CloudLight WeChat MCP Launcher] app.asar not found at "%MCP_ARCHIVE%"
  exit /b 1
)
