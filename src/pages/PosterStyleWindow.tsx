...
export default function PosterStyleWindow() {
  ...
  const handleGenerateAgentStyle = async () => {
    ...
    const prompt = `你是 CloudLight WeChat 海报样式设计 Agent。请为聊天记录分享海报生成一段 CSS 主题。
...
${sampleLines || '无'}`
  ...
  const handleFloppyDiskImage = async () => {
    ...
    try {
      ...
      link.download = `CloudLight-WeChat-聊天记录-${Date.now()}.png`
      link.href = dataUrl
      link.click()
      setStatusText('海报已保存')
    } catch (error) {
            ...
            {ordered.length === 0 ? (
              ...
              <div className={POSTER_THEME_SCOPE} ref={cardRef}>
                ...
                <div className="poster-card">
                  ...
                  <div className="poster-card__footer">由 CloudLight WeChat 导出</div>
                </div>
              </div>
            )}
          </div>
