...
// 把本地智能体的 activity 事件转成 CloudLight WeChat 标准的消息 part：思考→reasoning，工具/命令→tool-*。
...
export default function AgentPage() {
  ...
  useEffect(() => {
    return window.electronAPI.localCodingAgent.onEvent((event) => {
      ...
      if (event.type === 'message') {
        // 只有模型的最终回复进正文；CloudLight WeChat 的状态提示一律不入气泡。
  ...
  // 补丁应用/丢弃的结果属于 CloudLight WeChat 状态，走气泡外的提示条，不改动助手消息（否则会抹掉折叠链的思考/工具卡片）。
